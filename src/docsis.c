...
{
  unsigned char temp_hash[SHA_DIGEST_LENGTH], *buffer;
  ...
  memset(temp_hash, 0x0, SHA_DIGEST_LENGTH);
  
  SHA1((unsigned char *)tlvbuf, tlvbuflen, temp_hash);
  ...
  sprintf(mtahash,"Main\n{\n\tSnmpMibObject pktcMtaDevProvConfigHash.0 HexString 0x%s ;\n}\n", hash);
...
usage ()
{
  ...
  fprintf(stderr, "To decode a MTA config file with MTA config hash:\n"
		  "\tdocsis -hash -p <mta_cfg_file> <output_file>\n"
		  "\tdocsis -hash -m -p <mta_file1> ...  <new_extension>\n");
...
main (int argc, char *argv[])
{
  ...
  unsigned int encode_docsis = FALSE, decode_bin = FALSE, mta_hash = FALSE;
  int i;
  int resolve_oids = 1;
  ...
  } else if (!strcmp (argv[1], "-hash") ){ /* generte mta_hash */
        if (argc < 5 ) {
                usage();
	}
	mta_hash = 1;
...
int encode_one_file ( char *input_file, char *output_file,
	 		unsigned char *key, unsigned int keylen, int encode_docsis, int mta_hash )
{
  int parse_result=0;
  unsigned int buflen;
